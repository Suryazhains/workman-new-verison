        ...
        <div className="w-full max-w-[1920px] mx-auto px-5 md:px-10 xl:px-[90px]">
          ...
          <div className="hidden md:block bg-white rounded-3xl overflow-hidden shadow-2xl border border-white/10">
            <div className="relative flex bg-[#F0FDF4] border-b border-gray-100 no-scrollbar">
              ...
              />
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative z-10 flex-1 py-6 px-2 text-lg font-semibold transition-all outline-none border-none ${activeTab === tab.id ? 'text-[#FE4E5D]' : 'text-[#FE4E5D] hover:text-[#51A147]'}`}>
                  {tab.label}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 items-stretch">
              <div className="order-2 xl:order-1 px-10 pb-10 xl:px-[70px] xl:pt-[80px] xl:pb-[80px] flex flex-col justify-between animate-[fadeSlide_0.4s_ease-in-out]">
                <div className="space-y-6">
                  <h3 className="font-crimson text-3xl md:text-4xl font-bold text-[#FE4E5D]">
                ...
                <div className="pt-12">
                  <button onClick={() => handleViewMore(activeTab)} className="flex items-center gap-4 px-14 py-4 bg-[#FE4E5D] text-white text-base font-bold rounded-xl hover:bg-[#3E7D36] transition-all outline-none border-none shadow-lg shadow-green-900/20">
